...
namespace UIFramework.Reactive
{
    ...
    {
        private static readonly ILog _logger = LogManager.GetLogger(typeof(GreaterThanCondition));
        private UIElement _targetElement;
        private readonly string _property;
        private readonly IComparable _threshold;

        public GreaterThanCondition(UIElement target, string property, IComparable threshold) // funziona con int, double, DateTime, decimal, ecc.
        {
        ...
        public bool Evaluate()
        {
            ...
            _logger.Debug($"Evaluating GreaterThanCondition for property '{_property}', value '{value}' with threshold '{_threshold}' on element '{_targetElement.ToString()}'");
            var result = ComparableHelper.SafeCompareTo(value, _threshold) > 0;
            _logger.Debug($"GreaterThanCondition result for property '{_property}' on element '{_targetElement.ToString()}': {result}");
            return result;
        }
